        ...
        {data.rows.map(sale => {
          ...
          return (
            <div key={sale.siteId} className="flex border-b border-gray-200 bg-white hover:bg-gray-50 transition-colors">
              {/* Cover picture — the same SQUARE on every card, the picture shown whole inside it. The site's
                  cover files are square, so a whole picture fills the square exactly as vectis.co.uk's
                  archive shows them (Jordan, 2026-09-24: "the Pokémon one is the perfect example"); the odd
                  non-square file shows whole on white rather than cropped. ⚠ The box must never take its
                  height from the card's text — that is what made a two-line title's picture bigger than
                  its neighbours' and the column look ragged. */}
              <Link href={href} className="relative shrink-0 bg-white overflow-hidden" style={{ width: "220px", height: "220px" }}>
                {sale.photo ? (
