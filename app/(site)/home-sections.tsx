...
// The home page's sections, each a server component that fetches its own data — used by the
// built-in home page AND by the page editor's live blocks (page-editor/live.tsx), so there is one
// copy of each (2026-09-24).
...
/** The banner: the Banner Manager's live slides, sliding aside for a live auction when one is running. */
export async function HeroSection() {
  ...
  return <HomeHero initialLive={initialLive} dbSlides={dbSlides} isLoggedIn={!!session} />
}

/** The next published sales, as cards. Shows nothing when none is coming up. */
export async function UpcomingSection({ kicker = "Don't Miss", heading = "Upcoming Auctions", count = 6 }: { kicker?: string; heading?: string; count?: number }) {
  ...
  const upcoming = allPublished.filter(a => !isPast(a.auctionDate, a.finished, a.complete)).slice(0, Math.max(1, Math.min(24, Number(count) || 6)))
  if (upcoming.length === 0) return <div className="hidden" />
...
/** The stats band — the four figures, counted from the Hub's own tables (see site-stats.ts). */
export async function StatsSection() {
  // Always the same four — a count that isn't in yet shows as "—" rather than a made-up number or a gap.
...
/** The sale types as a strip of links into the calendar. */
export function SpecialismsSection({ kicker = "Explore by Category", heading = "Our Specialisms" }: { kicker?: string; heading?: string }) {
  return (
    <section className="bg-gray-50 py-14">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-10">
          {kicker && <p className="text-[#DB0606] text-xs font-black tracking-[0.25em] uppercase mb-2">{kicker}</p>}
          <h2 className="text-2xl font-black text-[#32348A] uppercase tracking-tight">{heading}</h2>
        </div>
...
/** "Why Choose Vectis?" — the built-in home page's three points (an editable Cards block in the editor). */
export function WhyVectisSection() {
  return (
...
/** "Ready to sell your collection?" — the built-in home page's call to action. */
