...
type Missing = { file: string }   // "dept-<slug>-hero|tile|<n>.<ext>" — the name the collector gave the picture

// The admin tools on the Departments page, as a chip (the same shape as the News page's): run the
// collector on an office machine (the website refuses the Hub's server), load the file it saved,
// then choose its pictures — each goes straight from the browser into R2 on a presigned PUT and is
// registered as the department's banner, tile or highlighted lot, with a count that moves and a Stop.
export default function DepartmentsTools({ missing, held }: { missing: Missing[]; held: number }) {
  ...
  const claudeText = [
    "Please collect the department pages from vectis.co.uk for the Vectis Hub — the 28 in the site's Departments menu, with their pictures. I will load them myself afterwards.",
    "",
    "Background: the Hub's Railway server cannot read the website (it answers 202 with an empty body), but a machine on the Vectis office network can.",
    "",
    "From C:\\Dev apps\\vectis-hub, run:",
    "",
    "    node scripts/collect-departments.mjs \"<my Downloads folder>/vectis-departments\"",
    "",
    "It writes vectis-departments.json, a pictures folder (banner, tile and highlighted lots per department) and a log beside them — about a minute for the pages and a few minutes for the pictures. It is safe to run again; pictures already in the folder are kept.",
    "",
    "When it finishes, tell me how many departments and highlighted lots it saved and whether any picture would not download — it names those at the end. Do not try to load anything into the database yourself.",
  ...
  // Which files in the chosen folder are department pictures the Hub still needs — by the file name
  // the collector gave them. Anything else in the folder (the news pictures, say) is ignored.
  ...
  async function upload() {
    ...
    try {
      ...
      const one = async (m: Missing) => {
        ...
        try {
          const r1 = await fetch("/api/databases/news/picture-url", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: 0, file: m.file, contentType, size: f.size }) })
          ...
          const r2 = await fetch("/api/databases/news/picture", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: 0, key: j1.key }) })
  ...
  return (
    <div className="space-y-2">
      ...
      {open && (
        <div className={box}>
          <div>
            <h3 className="text-sm font-bold text-gray-900 dark:text-white">📥 Update the departments from the website</h3>
            <p className="mt-1 text-gray-600 dark:text-gray-400">The website won&apos;t answer the Hub&apos;s server, so a machine in the office runs the collector and what it saves is loaded here. The Hub holds {held.toLocaleString()} department{held === 1 ? "" : "s"} now.</p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="text-gray-600 dark:text-gray-400">paste them into a Claude session on an office machine with the Hub&apos;s code — it runs <span className="font-mono text-xs">scripts/collect-departments.mjs</span>, which reads the 28 department pages and downloads their pictures into a folder.</span>
            </div>
          </div>

          <div className="space-y-2">
            <div className={step}>2 · Load the departments — vectis-departments.json</div>
            <div className="flex flex-wrap items-center gap-2">
              <input type="file" accept=".json,application/json" onChange={e => setFile(e.target.files?.[0] ?? null)} className="file-input" />
              <button type="button" onClick={load} disabled={!file || loading} className={`${btn} bg-violet-600 hover:bg-violet-500 text-white`}>{loading ? "Loading…" : "Load the departments"}</button>
            </div>
          ...
          <div className="space-y-2">
            ...
            {folder.length > 0 && !uploading && !finished && (
              <p className="text-xs text-gray-600 dark:text-gray-400">
                {folder.length.toLocaleString()} file{folder.length === 1 ? "" : "s"} chosen · {toUpload.length.toLocaleString()} match{toUpload.length === 1 ? "es" : ""} a department picture the Hub doesn&apos;t have
                {missing.length > toUpload.length ? ` · ${(missing.length - toUpload.length).toLocaleString()} wanted picture${missing.length - toUpload.length === 1 ? " isn't" : "s aren't"} among them` : ""}
              </p>
            )}
            {(uploading || finished) && (
