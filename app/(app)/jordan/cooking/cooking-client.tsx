...
export default function CookingClient() {
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 jsys-font" style={{ color: GREEN }}>
      ...
      {tab === "airfryer" && (
        <div className="flex-1 min-h-0 overflow-y-auto border border-(--j-dim) rounded-xl p-4 space-y-4">
          ...
          {/* ── My air fryer setup ── */}
          <div className="border border-(--j-dim) rounded-lg">
            ...
            {setupOpen && (
              <div className="px-3 pb-3 space-y-3 border-t border-(--j-dim)">
                <div className="pt-3">
                ...
                <div>
                  <div className="flex items-center justify-between gap-2 mb-1">
                    ...
                    <button onClick={() => scanInput.current?.click()} disabled={scanning}
                      className="text-[11px] px-2 py-1 rounded border border-(--j-acc) hover:bg-(--j-glow) disabled:opacity-40 transition-colors">
                  ...
                  {modes.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mb-2">
                      {modes.map((m) => (
                        <span key={m} className="inline-flex items-center gap-1.5 px-2 py-1 rounded-lg border border-(--j-acc) text-xs">
                          {m}
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      {suggestable.map((m) => (
                        <button key={m} onClick={() => addMode(m)}
                          className="px-2 py-0.5 rounded-lg border border-(--j-dim) text-[11px] opacity-60 hover:opacity-100 hover:border-(--j-acc) transition-all">+ {m}</button>
                      ))}
                    </div>
                  )}
                </div>
          ...
          <div className="flex gap-2 flex-wrap">
            <button onClick={() => fileInput.current?.click()} disabled={busy}
              className="px-4 py-2.5 rounded-lg border border-(--j-acc) text-sm font-bold hover:bg-(--j-glow) disabled:opacity-40 transition-colors">
              📷 CHOOSE / TAKE PHOTO
            </button>
            {file && (
              <button onClick={convert} disabled={busy}
                className="px-4 py-2.5 rounded-lg text-sm font-bold text-(--j-on-acc) disabled:opacity-40 transition-colors"
          ...
          {preview && (
            <img src={preview} alt="Your food" className="max-h-56 rounded-lg border border-(--j-dim)" />
          )}

          {error && <p className="text-sm text-red-400">✗ {error}</p>}

          {result && (
            <div className="border border-(--j-acc) rounded-xl p-4 space-y-2 text-sm max-w-md">
              <p className="text-xs opacity-60 uppercase tracking-widest">— ANALYSIS COMPLETE —</p>
              <p className="text-base font-bold text-(--j-hi)">{result.food}{result.state && result.state !== "unsure" ? ` (${result.state})` : ""}</p>
              {!result.confident && <p className="text-xs text-amber-400">⚠ Not 100% sure what this is — sanity-check the settings.</p>}
              {result.mode && (
                <div className="border border-(--j-acc) rounded-lg p-3 text-center bg-(--j-glow)">
              ...
              <div className="grid grid-cols-2 gap-2 pt-1">
                <div className="border border-(--j-dim) rounded-lg p-3 text-center">
      ...
      {tab === "chat" && (
        <div className="flex-1 min-h-0 border border-(--j-dim) rounded-xl p-4">
          <ChatPanel
            mode="cooking"
