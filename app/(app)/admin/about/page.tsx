...
const APPS: App[] = [
    ...
    howItWorks: [
      {
        ...
        items: [
          ...
          "Select an auction code. The system loads all lots for that auction and matches photos to them using three-way lot lookup (lotNumber, barcode, receiptUniqueId).",
      ...
      },
      {
        ...
        items: [
          ...
          "Shape: Array<{ Folder, 'Receipt Unique ID', Barcode, 'Lot Number', Description, Estimate }>.",
          "Folder must always be receiptUniqueId || lotNumber || '' — never just lotNumber (Apply-to-Auction lots have empty lotNumber and would appear blank).",
          "Sort modes: Unique ID (parses receipt number and line number for correct numeric order), Barcode (alphanumeric), Lot Number (integer with alphanumeric fallback).",
      ...
      },
      {
        ...
        items: [
          ...
          "Scoring to identify the most complete record: description +4, title +2, keyPoints +1, estimateLow +1, estimateHigh +1, lotNumber +1, barcode +1, vendor +1, each image +2.",
  ...
  // ─── CATALOGUING ──────────────────────────────────────────────────────────
  {
    ...
    howItWorks: [
      ...
      },
      {
        ...
        items: [
          ...
          "Barcode matched via three-way lookup (lotNumber, barcode, receiptUniqueId) — case-insensitive, trimmed.",
      ...
      },
      {
        ...
        items: [
          ...
          "Three-way lot lookup checks all three identifier fields.",
        ],
      },
      {
      ...
      },
      {
        ...
        items: [
          ...
          "Detects unique ID format /^[A-Za-z]\\d{4,7}-\\d{1,6}$/ → receiptUniqueId = lot, lotNumber = '' (empty is correct).",
          "Otherwise → lotNumber = lot, receiptUniqueId = null.",
          "Deduplication: checks existingLotNumbers and existingUniqueIds sets before creating. Returns count of created + skipped.",
        ],
      },
      {
        ...
        items: [
          ...
          "assignLotNumbers() — sequential numbering from ordered ID array.",
    ...
    rules: [
      "Three identifier fields (receiptUniqueId, barcode, lotNumber) are NOT interchangeable. Never store a unique ID in lotNumber.",
      "Unique ID format: /^[A-Za-z]\\d{4,7}-\\d{1,6}$/ e.g. R000016-413. Barcode format: /^[A-Za-z]\\d{6,7}$/ e.g. F066001.",
      "Lots from Apply to Auction have empty lotNumber — correct and expected.",
      ...
      "Lot lookup map checks all three identifier fields (case-insensitive, trimmed).",
      "Description Copier: Folder = receiptUniqueId || lotNumber || '' — never just lotNumber.",
    ...
    howItWorks: [
      ...
      },
      {
        ...
        items: [
          ...
          "Current lot: id, lotNumber, title, description, imageUrls[], estimateLow, estimateHigh, status, currentBid, askingBid, increment, hammerPrice, bids[].",
    ...
    howItWorks: [
      ...
      },
      {
        ...
        items: [
          ...
          "CatalogueLot: id, lotNumber, barcode, receiptUniqueId, title (83 char max), description (TEXT), keyPoints, estimateLow, estimateHigh, startingBid, reserve, currentBid, hammerPrice, condition, vendor, tote, receipt, category, subCategory, brand, notes, extraDetails (TEXT), imageUrls[], status (ENTERED/REVIEWED/PUBLISHED/SOLD/UNSOLD/WITHDRAWN), aiUpgraded, createdByName, auctionId.",
          "CatalogueTimingLog: auctionId, lotId, userId, userName, method (WIZARD/PHOTO_ONLY), durationMs, keyPointsMs, lotNumber, savedAt.",
