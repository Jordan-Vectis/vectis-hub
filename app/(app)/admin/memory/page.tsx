...
## The BC-only table — LIVE from Receipt Totes (SETTLED 2026-07-29)
...
**Metric (Jordan's final):** "Using totes from" = the **MEDIAN check-in month of the newest 10 benched (PTE_Benched=true) totes in each category** ("Mar 2026"). ⚠ Median NOT trimmed-mean — the benched set per category can be small, and a trimmed mean can't trim a tiny sample so a stray outlier skewed it; median is robust at any n. Sample is "up to 10" — if a category shows fewer, that's genuinely all the benched totes the live feed holds for it (the fetch now pulls the whole feed, so it's not an under-fetch).

**⚠⚠ LIVE from BC's \`Receipt_Totes_Excel\`, not our synced data.** That feed has per tote: \`EVA_TOT_ArticleCategory\`, \`SystemCreatedAt\` (check-in), and **\`PTE_Benched\`** — Jordan: **PTE_Benched=true is the reliable "catalogued/worked" signal; EVA_TOT_Catalogued is unreliable.** ⚠⚠ **FETCH SHAPE (SETTLED 2026-07-29 — the proven pattern, matches the working BC Warehouse tab \`app/api/bc/warehouse/route.ts\`):** pull the WHOLE feed in ONE call — \`bcFetchAll(token, "Receipt_Totes_Excel")\` with NO \`$filter\` and NO \`$select\` — then group by \`EVA_TOT_ArticleCategory\` and filter \`PTE_Benched\` **in code**, newest-10 by SystemCreatedAt, median. The feed is small (low thousands, under the 38k $skip limit) and the raw pull contains BOTH catalogued and uncatalogued totes (bc/warehouse proves this — it filters \`uncat\` in code after the same full pull). Category list is the feed's own categories; counts + "Last worked" still from WarehouseItem, joined by normalised name. Counts/"Last worked" only; dates are live. **Two dead ends that UNDER-fetched, do NOT retry:** (1) filtering the whole feed + server paging (BC emits no nextLink → only first 500 rows, GAMING got 1); (2) ONE \`$filter=EVA_TOT_ArticleCategory eq '<cat>'\` query per category (still under-returned, GAMING got 5). Also NEVER \`$filter\` on PTE_Benched — it's a BC FLOW field, OData returns the wrong subset.

**⚠ Why live (the wall that ate ~6 rounds):** our synced WarehouseTote didn't reliably hold the benched/catalogued totes' dates, so no maths on synced data could fix it — the live feed is the source of truth. (Earlier note "BC drops catalogued totes from the feed" is imprecise: the LIVE feed keeps them; only our sync's copy lagged.)

**⚠ Dead ends, do NOT retry:** keying by our departments; "frontier"/newest-catalogued MAX (cataloguing is OUT OF ORDER → nonsense); filtering WarehouseTote.catalogued (all-false — strict parse vs BC string; bcBool fixes the parse but the table doesn't use it); linking catalogued items by toteNo (items lose it); WarehouseContainer (internal warehouse, different system). All BC date parsing guards >= 1990 (BC empty date = 0001-01-01).
