...
**Metric (Jordan's final):** "Using totes from" = the **MEDIAN tote check-in month of the newest 10 CONSIGNMENTS catalogued in each category** ("Jun 2026"). Group catalogued \`WarehouseItem\` by \`receiptNo\` (a receipt ≈ a tote/consignment — items don't carry a tote no, \`EVA_ArticleToteNo\` empty on ~all), rank by \`MAX(cataloguedAt)\` desc, take top 10, date each receipt via **\`WarehouseTote.bcCreatedAt\` joined on receiptNo** (MIN per receipt, guard >= 1990). ⚠ Median NOT trimmed-mean (a small sample can't be trimmed; median is robust at any n). ⚠⚠ **NOT \`goodsReceivedDate\` — verified 2026-07-29 on production: populated on 0 of ~208k WarehouseItem rows, a DEAD field** (the departments-view comment calling receiptNo→goodsReceivedDate "the path that works" was stale — its working path is tote bcCreatedAt). Undated receipts (tote never synced with bcCreatedAt) are excluded from the median and shown as chips; coverage improves as the nightly totes-active sync accumulates bcCreatedAt.
