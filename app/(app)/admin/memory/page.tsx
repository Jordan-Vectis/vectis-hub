...
**⚠⚠ LIVE from BC's \`Receipt_Totes_Excel\`, not our synced data.** That feed has per tote: \`EVA_TOT_ArticleCategory\`, \`SystemCreatedAt\` (check-in), and **\`PTE_Benched\`** — Jordan: **PTE_Benched=true is the reliable "catalogued" signal; EVA_TOT_Catalogued is unreliable.** ⚠⚠ PTE_Benched is a BC FLOW field — CANNOT $filter on it (OData returns wrong subset). AND paging the whole unfiltered feed failed (BC gave no nextLink → only first 500 rows → GAMING got 1). Working shape: ONE query PER CATEGORY (\`$filter=EVA_TOT_ArticleCategory eq '<cat>'\`, a real filterable field), page that small result fully, keep benched IN CODE, newest-10 by SystemCreatedAt, median. Categories in parallel (concurrency 4). Counts + "Last worked" still from our WarehouseItem; only the dates are live.
