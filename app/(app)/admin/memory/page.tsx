...
⚠⚠ **Catalogued items LOSE their toteNo — join by RECEIPT.** The first version keyed off \`WarehouseItem.toteNo\` and showed "nothing catalogued" for categories with thousands done, while "Last catalogued" worked. Proof: cataloguedAt is valid, so the failing filter was toteNo — **BC clears an item's toteNo once it's catalogued**. receiptNo survives, and \`WarehouseTote\` keeps receiptNo + \`bcCreatedAt\`, so the table is keyed by receipt: last 10 receipts catalogued per category, each dated via \`WarehouseTote.receiptNo\` → \`bcCreatedAt\` (else \`WarehouseItem.receiptNo\` → \`goodsReceivedDate\`). \`bcStockFor\` is standalone. Panel shows receipt numbers. ⚠ Still needs the totes-active sync run to fill \`bcCreatedAt\`. All BC date queries carry the \`>= DATE '1990-01-01'\` guard.
