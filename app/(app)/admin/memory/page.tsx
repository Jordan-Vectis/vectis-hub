...
**⚠⚠ LIVE from BC's \`Receipt_Totes_Excel\`, not our synced data.** That feed has per tote: \`EVA_TOT_ArticleCategory\`, \`SystemCreatedAt\` (check-in), and **\`PTE_Benched\`** — Jordan: **PTE_Benched=true is the reliable "catalogued" signal; EVA_TOT_Catalogued is unreliable.** ⚠⚠ PTE_Benched is a BC FLOW field — CANNOT $filter on it (OData returns wrong subset). AND BC emits NO nextLink on Receipt_Totes_Excel → nextLink paging only ever returns the first 500 rows (GAMING got 1, then 5). Must paginate by $skip — use bcFetchAll. Working shape: ONE bcFetchAll PER CATEGORY (\`$filter=EVA_TOT_ArticleCategory eq '<cat>'\`, a real filterable field, well under the 38k $skip limit), fetch ALL that category's totes, keep benched IN CODE, newest-10 by SystemCreatedAt, median. Categories in parallel (concurrency 4). Counts + "Last worked" still from our WarehouseItem; only the dates are live.

**⚠ Why live (the wall that ate ~6 rounds):** our WarehouseTote only holds UNCATALOGUED totes (BC drops a tote from the feed once catalogued), so catalogued-tote dates aren't in our copy — MILITARY resolved only 2 of 10, both outliers → nonsense. No maths on synced data could fix it.
