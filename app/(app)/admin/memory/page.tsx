...
## The BC-only table — from WarehouseItem (SETTLED 2026-07-29)

One row per **BC main category** (\`EVA_ArticleCategoryCode\` via \`WarehouseItem.category\`), every category BC holds, **nothing** from our CatalogueAuction/Lot/Department. Component \`BcCategoryTable\` (departments-table.tsx); route **\`/api/manager-portal/bc-tote-dates\`** (DB-only now); the client fetches it async. No "% done". "Last worked" = MAX(WarehouseItem.cataloguedAt); Catalogued/Still-to-do = item counts by category.

**Metric (Jordan's final, tote-level 2026-07-29):** "Using totes from" = the **MEDIAN check-in month of the newest 10 TOTES catalogued in each category** ("Jun 2026"). Group catalogued \`WarehouseItem\` by **\`toteNo\` — the item's SOURCE TOTE from \`EVA_CFA_TOT_CreatedFromToteNo\`** (CONFIRMED populated in BC incl. old receipts; \`EVA_ArticleToteNo\` is the empty one; the receipt-lines sync maps it since 2026-07-29 — **a full Receipt Lines re-sync backfills old rows**, until then they fall back to receipt grouping keyed \`R:<receiptNo>\`). **BENCHED FILTER (Jordan 2026-07-30): a tote only counts if it reached a cataloguing bench** — WarehouseTote keeps each tote's LAST-KNOWN location ("BENCH34"…), so keep totes whose location contains BENCH **or is unknown/blank** (cleared = finished = was worked); exclude only totes KNOWN to sit on a shelf (a stray ticked item on a shelved tote must not drag an ancient tote into the sample). Receipt-fallback groups can't be assessed, kept. Then rank groups by \`MAX(cataloguedAt)\` desc, top 10, date each tote via **\`WarehouseTote.bcCreatedAt\` joined on toteNo** (receipt-fallback via MIN over the receipt's totes; guard >= 1990). ⚠ Median NOT trimmed-mean (a small sample can't be trimmed). ⚠ Date coverage bounded by capture: bcCreatedAt only exists for totes still in the feed on/after 2026-07-27; grows nightly. **Totes with no captured check-in get an ESTIMATED date interpolated from the tote NUMBER sequence** (tote numbers are allocated chronologically — verified: nearest dated neighbours bracket a tote within ~1–2 days; ~1,700 anchors across the separate T and P sequences). Estimates are flagged \`estimated\`, shown as "~9 Mar 26" with a tooltip, included in the median (a median off 1–2 real dates was wildly wrong — MILITARY read Sept 2025 off a single tote; with estimates it reads ~Mar 2026, matching Jordan's expectation). Receipt-fallback rows are never estimated. ⚠⚠ **NOT \`goodsReceivedDate\` — populated on 0 of ~208k WarehouseItem rows, a DEAD field.** Undated totes are excluded from the median and shown as chips.

**⚠⚠ WHY NOT the live tote feed (the wall that ate ~8 rounds — MEASURED 2026-07-29).** The obvious source — \`Receipt_Totes_Excel\` filtered to \`PTE_Benched\` — CANNOT work, for TWO measured reasons: **(1)** the feed is small and current-state only (1,776 rows; 279 benched, retained only for a while after cataloguing) — finished-tote history isn't there; **(2)** ⚠⚠ **the tote line's category label ≠ the items' categories** — R008868's benched tote is labelled TV_FILM while its items are GAMING, receipts hold mixed stock, so "benched GAMING totes" by tote label found 5 vs 1,729 catalogued GAMING items. NOT an under-fetch ($skip pagination verified working; the full pull was complete). So the table uses our COMPLETE item-level \`WarehouseItem\` data for the sample (item categories) + our accumulated \`WarehouseTote\` for the dates — DB-only, no live BC, can't run dry. (Our WarehouseTote is an upsert ACCUMULATOR — synced totes stay after they leave the feed — which is exactly why it can date finished consignments the live feed can't.)

**⚠ Dead ends, do NOT retry:** the live \`Receipt_Totes_Excel\`/\`PTE_Benched\` feed (active-only, missing finished totes — see above); \`WarehouseItem.goodsReceivedDate\` (0 of ~208k populated — every category showed "no goods-received dates"); keying by our departments; "frontier"/newest-catalogued MAX (cataloguing is OUT OF ORDER → nonsense); filtering WarehouseTote.catalogued (all-false + it lacks finished totes' dates too); linking catalogued items by toteNo (items lose it); WarehouseContainer (internal warehouse, different system). All date parsing guards >= 1990 (BC empty date = 0001-01-01).
...
- **⚠⚠ MEASURED 2026-07-29 (direct full pull): 1,776 rows total, 279 benched; $skip pagination WORKS here; natural order starts at ANCIENT backlog receipts (R000009…).** The feed = the current warehouse state: uncatalogued backlog + recently-benched totes (benched rows are retained for a while, then leave). It is NOT a source for finished/historical tote history.
- **⚠⚠ The tote line's category label ≠ the items' categories.** R008868's tote is labelled TV_FILM in the feed while its items are GAMING in WarehouseItem — receipts hold mixed stock, so counting "benched GAMING totes" by \`EVA_TOT_ArticleCategory\` found 5 vs 1,729 catalogued GAMING items. **Group by ITEM category for any per-category stock/age metric, never by the tote label.** Also \`EVA_TOT_CataloguedAt\` is the 0001 sentinel even on benched rows (can't rank by finished-time). For "how far behind / stock age per category" use our DB: newest-catalogued WarehouseItems grouped by receiptNo, dated via **WarehouseTote.bcCreatedAt joined on receiptNo** (our WarehouseTote is an upsert accumulator so it keeps totes after they leave the feed). ⚠ NOT WarehouseItem.goodsReceivedDate — populated on 0 of ~208k rows (verified 2026-07-29), a dead field. PTE_Benched (bool) = the "on a bench/worked" signal (reliable, vs EVA_TOT_Catalogued which is unreliable), but it's a **flow field — NEVER $filter it** (OData returns wrong subset), test in code.
