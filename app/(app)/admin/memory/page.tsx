...
WINDOWS OCR NEVER RETURNS A LONE SINGLE CHARACTER (measured 2026-08-21): a crisp "5" alone in the Saleroom H box reads as nothing - scaled, bordered, tiled or prefixed from a distance, always empty - while "15", "1,250" and the full line "Current Bid: 5" read fine; glued-suffix tricks were inconsistent, do not revisit. Fix: the Saleroom profile has an extra A-box region (reg_ask); ReadBid reads H, and when that is empty reads the next asking and steps back one increment on the saleroom ladder (10 -> 5, 60 -> 50). On the Saleroom trainer H is the current bid and A the next asking. Also added "Set just one" - a dropdown of the screen's positions with a button that calibrates only that one, so changing a button no longer means redoing them all.

PITFALLS HIT (do not repeat): top-level "global X := 0" lines run in FILE ORDER at start-up wherever they sit - the window globals were declared below the BuildMainGui() call, executed after the window existed and reset its references to 0 ("Calibrate does nothing", fixed 2026-08-21 by moving every declaration above the start-up calls, verified with a driver script). AHK built-in names as variables fail to LOAD - RUN (the Run command) and Log( (maths) - renamed RS / WriteLog; a load error leaves a dialog open so headless tests hang - validate first. PowerShell: inside New-Object Type($a * $b, $c * $d) the comma binds tighter than * ("op_Multiply" on Object[]) - compute first, use -ArgumentList. A killed .ahk does not kill its PowerShell child - StopOcr sends quit, waits, then closes; find strays with wmic on the command line.
