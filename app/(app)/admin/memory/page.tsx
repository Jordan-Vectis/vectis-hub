...
**Metric (Jordan's final, tote-level 2026-07-29):** "Using totes from" = the **MEDIAN check-in month of the newest 10 TOTES catalogued in each category** ("Jun 2026"). Group catalogued \`WarehouseItem\` by **\`toteNo\` — the item's SOURCE TOTE from \`EVA_CFA_TOT_CreatedFromToteNo\`** (CONFIRMED populated in BC incl. old receipts; \`EVA_ArticleToteNo\` is the empty one; the receipt-lines sync maps it since 2026-07-29 — **a full Receipt Lines re-sync backfills old rows**, until then they fall back to receipt grouping keyed \`R:<receiptNo>\`). Rank groups by \`MAX(cataloguedAt)\` desc, top 10, date each tote via **\`WarehouseTote.bcCreatedAt\` joined on toteNo** (receipt-fallback via MIN over the receipt's totes; guard >= 1990). ⚠ Median NOT trimmed-mean (a small sample can't be trimmed). ⚠ Date coverage bounded by capture: bcCreatedAt only exists for totes still in the feed on/after 2026-07-27 (earlier-cleared totes aren't in WarehouseTote at all — e.g. T026383 worked Jul 2026, missing); grows nightly, don't chase the pre-27-Jul gaps. ⚠⚠ **NOT \`goodsReceivedDate\` — populated on 0 of ~208k WarehouseItem rows, a DEAD field.** Undated totes are excluded from the median and shown as chips.
