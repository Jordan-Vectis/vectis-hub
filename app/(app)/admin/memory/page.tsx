...
**⚠⚠ WHY NOT the live tote feed (the wall that ate ~8 rounds).** The obvious source — \`Receipt_Totes_Excel\` filtered to \`PTE_Benched\` — CANNOT work: **that's BC's ACTIVE-totes feed; a tote drops out once finished/shelved.** GAMING had **1,729 catalogued items but only 5 benched totes in the feed** — no fetch pattern fixes that, the finished totes aren't in the feed at all (the fuller view Jordan sees in the BC client isn't on this OData endpoint). So the table now uses our COMPLETE item-level \`WarehouseItem\` data for the sample + our accumulated \`WarehouseTote\` for the dates — DB-only, no live BC, can't run dry. (Our WarehouseTote is an upsert ACCUMULATOR — synced totes stay after BC drops them from the feed — which is exactly why it can date finished consignments the live feed can't.)
...
- **⚠⚠ ACTIVE totes only — a tote DROPS OUT once finished/shelved.** So it is NOT a source for finished/historical catalogued totes (GAMING: 1,729 catalogued items but only 5 benched totes in the feed). For "how far behind / stock age per category" use our DB instead: newest-catalogued WarehouseItems grouped by receiptNo, dated via **WarehouseTote.bcCreatedAt joined on receiptNo** (our WarehouseTote is an upsert accumulator so it keeps totes BC has dropped). ⚠ NOT WarehouseItem.goodsReceivedDate — populated on 0 of ~208k rows (verified 2026-07-29), a dead field. PTE_Benched (bool) = the "on a bench/worked" signal (reliable, vs EVA_TOT_Catalogued which is unreliable), but it's a **flow field — NEVER $filter it** (OData returns wrong subset), test in code.
