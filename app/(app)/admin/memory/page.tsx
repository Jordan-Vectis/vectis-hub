...
Correct shape: one row per **BC main category** (\`WarehouseItem.category\` = \`EVA_ArticleCategoryCode\`), covering **every category BC holds** whether or not we have a sale or department for it. Component \`BcCategoryTable\`; row type \`BcCategoryRow\`.

⚠ **"Using totes from" = the FRONTIER**, not a median. Jordan reads it off BC's Receipt Totes screen: sort by Created At, the newest **catalogued** tote is where cataloguing has reached (GAMING = 11 May; everything newer is unticked). So it's **MAX(bcCreatedAt) among catalogued totes per category**. ⚠⚠ Catalogued state comes from the **ITEM**, not the tote. A version filtering \`WarehouseTote.catalogued = true\` showed "nothing catalogued" everywhere even after a full resync — \`WarehouseTote.catalogued\` was stored all-false because the totes-active sync parsed \`EVA_TOT_Catalogued\` with strict \`=== true\` but BC sends it as a STRING ("Yes"). Fixed the parse (\`bcBool\`) but the report now drives catalogued state off \`WarehouseItem.catalogued\` (the working signal): a \`cat_receipt\` CTE of receipts with a catalogued item + category, joined to \`WarehouseTote\` ONLY for bcCreatedAt; newest 10 totes kept, row 1 = frontier. ⚠ Don't reintroduce a WarehouseTote.catalogued filter. ⚠ Frontier is TRIMMED not a pure MAX: \`bcStockFor\` ignores the newest \`BC_FRONTIER_TRIM\`=2 catalogued totes (a single out-of-order tick would drag it to today) and takes the next; ignored totes shown struck-through in the panel. ⚠ FINAL (2026-07-29): median check-in of the LAST 15 RECEIPTS CATALOGUED — NOT a frontier. Cataloguing runs out of order (MILITARY: stray April ticks over a dense mid-March block), so 'newest catalogued' is meaningless (gave a nonsense 3 Sept). Metric: last 10 receipts catalogued (by MAX(WarehouseItem.cataloguedAt) per receipt), each dated by its check-in, then a single AVERAGE MONTH = trimmed mean (drop ~20% each end) 'Mar 2026'. ⚠⚠ Date source = item goodsReceivedDate FIRST, tote bcCreatedAt fallback, LEFT JOIN so all 10 return. Killer bug: dating via WarehouseTote only (inner join) returned just 2 of MILITARY's 10 - Receipt_Totes_Excel mostly holds UNCATALOGUED totes, so a catalogued receipt has no tote there; the 2 were outliers -> nonsense Feb. Panel shows all 10 + src (rcvd/tote) + 'X of Y dated'. Evolution: median-of-15 (too old) -> last-10 range (outliers made it silly) -> trimmed-mean single month (removes old stragglers, lands ~March). Sample/trim are one-line tweaks. Uses item.catalogued/cataloguedAt + bcCreatedAt — no WarehouseTote.catalogued, no resync. ⚠ Do NOT return to a frontier/newest/MAX metric — that burned ~5 rounds (WarehouseTote.catalogued=all-false; per-tote MAX scattered to Nov 25; per-receipt MIN gave 3 Sept). Panel lists the 15 receipts + their date span. ⚠ **"Last worked"** column (renamed from "Last catalogued") = MAX(cataloguedAt) = when work last happened (15 Jun) — a different metric, don't conflate. ⚠ No **"% done"** (removed). ⚠ Dead end: catalogued items LOSE their toteNo, so source catalogued state from the TOTE, not the item. Needs the totes-active sync to fill bcCreatedAt; all BC date queries carry the \`>= DATE '1990-01-01'\` guard.
