...
⚠ **Sliders are ABSOLUTE minutes with pinning, NOT proportional shares** (v1 used shares and Jordan rejected it — adjusting one slider moved values he'd already set): a dragged slider is PINNED exactly where set; untouched reasons flex to absorb the remainder; drags are capped so every reason keeps a 30s minimum; if ALL are pinned, a drag trades time with the pin set longest ago. Thumb position = actual time (max = the gap; step 15/30/60s by gap size). Segments always sum exactly to the gap. Per-reason note fields (requiresNotes / notePrompt each get their own, prefixed with the reason name when several are picked); the Lunch-Break >65-min mandatory note keys off **lunch's allocated share**, not the whole gap; totes field shows when Lotting Up is among the picks.
