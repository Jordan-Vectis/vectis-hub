...
**⚠⚠ LIVE from BC's \`Receipt_Totes_Excel\`, not our synced data.** That feed has per tote: \`EVA_TOT_ArticleCategory\`, \`SystemCreatedAt\` (check-in), and **\`PTE_Benched\`** — Jordan: **PTE_Benched=true is the reliable "catalogued" signal; EVA_TOT_Catalogued is unreliable.** ⚠⚠ PTE_Benched is a BC FLOW/calculated field — you CANNOT $filter on it (OData returns the wrong subset: 4 MILITARY totes when BC shows many). Route pages the WHOLE Receipt_Totes_Excel feed via bcPageWithNext (nextLink) and tests PTE_Benched in code. Counts + "Last worked" still from our WarehouseItem; only the dates are live.
