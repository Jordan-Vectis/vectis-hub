...
**Metric (Jordan's, and it is genuinely this simple — it mirrors his own BC view: Receipt Totes → filter Article Category + Location \`BENCH*\` → sort Created At desc):** pull the WHOLE \`Receipt_Totes_Excel\` feed → group by \`EVA_TOT_ArticleCategory\` → **keep totes whose \`EVA_TOT_ToteLocation\` contains BENCH** → newest 10 by \`SystemCreatedAt\` (check-in) → **median = the month**. Verified 2026-07-30 to reproduce his screenshot row-for-row (TRAINS 74 on benches, newest 10 all Jun/Jul 26 → Jun 2026). ⚠ Use the **LOCATION**, not \`PTE_Benched\` (Jordan's call; catches more — TRAINS 74 vs 42). ⚠ Median NOT trimmed-mean (small samples can't be trimmed). Bench counts are genuinely small for some categories (MILITARY 4, SPORTS 2) — that's real, not an under-fetch. Chips show tote no · check-in date · which bench.
...
**⚠⚠⚠ THE CEILING ON THIS TABLE — read before "fixing" a thin category (measured 2026-07-30).** BC's published \`Receipt_Totes_Excel\` web service **returns ONLY totes that are NOT ticked Catalogued** — 1,776 rows, every one \`EVA_TOT_Catalogued = false\`. Totes already ticked off are **absent entirely**: a \`$skip\` walk collects all 1,776 DISTINCT rows (complete, not a paging bug), unfiltered \`@odata.count\` = 1,776, and direct lookups of catalogued totes visible on BC's own page (T026013, T025980, T025902 …) **return nothing**. So SPORTS shows **3** totes on a bench where BC's page shows ~**30**. \`$filter\` on \`EVA_TOT_Catalogued\` is ignored too (true and false both return all 1,776). **FIX = publish an UNFILTERED Receipt Totes web service in BC, then swap \`FEED_ENDPOINT\` in the route (one line).** No other endpoint exists (11 names probed → 404; the OData service root lists 0 services). Our \`WarehouseTote\` can't backfill it — those totes were ticked before we ever synced them.

**Debugging built in (2026-07-30, Jordan asked for it).** Each month shows its **sample size, amber under 10**, and a **"Where these numbers come from"** expander gives the endpoint, rows pulled, the limitation in plain English, and a per-category table (totes in feed / on a bench / used for the month, thinnest first). Route returns a \`diagnostics\` block feeding it. So a thin category explains itself instead of silently under-reporting.

**Hiding categories (2026-07-30).** Admins get a hover **✕ Hide** per row and a "Hidden from this table" strip underneath (who hid it + ↺ Restore) — new \`ManagerPortalHiddenCategory\` table, \`toggleHiddenBcCategory\` in \`lib/actions/manager-portal.ts\`, same restorable display-only pattern as the report exclusions. The route filters hidden ones out and returns them + \`isAdmin\`; a missing table reads as "nothing hidden" so the deploy can't 500 before migrations run.
