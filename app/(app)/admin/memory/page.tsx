...
**Home page look (2026-09-24, Jordan's list, all done):** the top row runs to the page EDGES (search hard left, account hard right, logo 250px — it was mathematically centred already; the unequal side blocks made it look off); the hero shows the picture at full strength under a left→right dark gradient (never a flat wash); slide dots only with 2+ slides (a lone one looked like a stray mark); "Est. 1988" (was 1995 — the company-facts file); the stats band is COUNTED in app/(site)/site-stats.ts (lots with a hammer across ArchiveLot + WarehouseItem, sales in the last 365 days from ArchiveSale, SiteDepartment count; cached 6 h in-process, 4 s timeouts, a stat that can't be counted is LEFT OFF — never typed-in numbers like "180+ countries", which was also in the Why Vectis copy); HeroSlide.imageFocus top/center/bottom (**NEEDS Run Migrations**; nullable, no default; both reads tiered) = the Banner Manager's "Keep in frame" buttons; nav CAREERS → /careers (page existed), CONTACT US → new /contact (company facts only), SELL WITH US → /sell-with-us.
