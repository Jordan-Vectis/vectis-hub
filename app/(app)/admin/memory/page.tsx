...
WINDOWS OCR NEVER RETURNS A LONE SINGLE CHARACTER (measured 2026-08-21): a crisp "5" alone in the Saleroom H box reads as nothing - scaled, bordered, tiled or prefixed from a distance, always empty - while "15", "1,250" and the full line "Current Bid: 5" read fine; glued-suffix tricks were inconsistent, do not revisit. Fix: the Saleroom profile has an extra A-box region (reg_ask); ReadBid reads H, and when that is empty reads the next asking and steps back one increment on the saleroom ladder (10 -> 5, 60 -> 50). On the Saleroom trainer H is the current bid and A the next asking. Also added "Set just one" - a dropdown of the screen's positions with a button that calibrates only that one, so changing a button no longer means redoing them all. Test read (reworked once several boxes existed) walks EVERY calibrated box on the selected screen - each box's own enlarged picture, what Windows read and what it was taken to mean - then the verdict the clerk would act on, with the H-to-A fallback explained, a red warning while the A box is unset, and a note while the tie-check label boxes are missing.
