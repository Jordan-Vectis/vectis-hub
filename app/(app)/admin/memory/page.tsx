...
- **Unpacked items (same day, Jordan):** photography takes the loco and its inner packaging OUT of the box and lays them beside it, so a box + inner cradle + model reads as two or three items. The prompt now says: those together are ONE boxed item; an empty outer box, a loose inner tray and a model of the thing named on the box belong together; the box artwork may show a different livery/variant from the model inside — the model (and the key points) is what is sold, the artwork is never an item.
...
**Test it on the SANDBOX, not staging** (vectis-hub-sandbox.up.railway.app): Jordan reset the sandbox database from main on 2026-09-23 so it holds production's data — the sale databases are on production only, so staging's site shows nothing under View Results.

**⚠ The site inherits the Hub's dark theme.** html class="dark" is the Hub's default and globals.css paints every input/select/textarea dark for it with an UNLAYERED rule — which beats the site's Tailwind classes (bg-transparent, text-gray-800), so the search boxes came out black on the white site (2026-09-23). app/(site)/site.css (imported by the site layout, scoped to the vectis-site class on its wrapper, color-scheme: light) paints them back. Any new site control gets that for free; never fix it in globals.css.

**⚠ "Nothing happens, then 3 seconds later it moves" (2026-09-24, department pages):** every site page is force-dynamic, so a click shows nothing until the server answers — and the department page's "our results page for this highlighted lot" lookup (ourLotLinks) searched ArchiveLot (956k rows) and BcLotWeb by siteLotId with NO index → two full scans per page. Fixed three ways, keep all three: @@index([siteLotId]) on both (in the MIGRATIONS array — needs Run Migrations); the lookup runs in a transaction with SET LOCAL statement_timeout = 1500 and falls back to the website's links; and app/(site)/loading.tsx shows a bar + "Loading" the moment any site link is followed. Same day: the nav's hover dropdowns are now components/site-nav-dropdown.tsx (client) and CLOSE when a link inside is chosen — a pure hover menu stayed open over the new page; and the top bar is a grid-cols-[1fr_auto_1fr] so the logo is centred (justify-between had it drifting toward the narrower side). Admins get a floating "← Back to the Hub" pill (hub-return.tsx, auth() in the site layout, hidden per tab with ×). Department pages are wide (max-w-[1800px]) — "mainly for SEO", so the copy is two-thirds, the side box one-third, and generateMetadata gives a description from the copy's first lines.

**⚠ Banner Manager pictures were broken (2026-09-24):** it uploaded through the generic /api/upload-url, which files everything under submissions/ (the Submissions app's storage), and showed them through /api/public/photo, which serves only lot-photos/, catalogue-photos/, first-aid/, site-plans/ — so every banner sat in R2 and came back broken. Now: its own admin route POST /api/website/banner-upload (images only, key hero-slides/<stamp>-<rand>.<ext>, returns a signed viewUrl too), the banner page and the site's home hero attach a **signed URL** per slide (imageUrl) and render plain img tags — old submissions/ keys show that way too, no re-upload needed. Never widen the public proxy's prefixes for this. Upload failures now show a message (they were swallowed).

**⚠ News categories ARE the tags (2026-09-24):** the feed's Joomla catid is one container for every article (catid 8 "TV & FILM", 1,337 of 1,337); the site's category list (getCategories) is its tag list. The collect route sets category = tags[0], and every category filter (Hub News page, site list, article "more", department news) matches ANY(tags); category pickers come from unnest(tags). 201 articles carry no tag at all. Fixed by RELOADING the parts — no re-collection needed.

**⚠ Pictures that were the site's error page (2026-09-24):** 9 covers whose site paths carry %20 were fetched with the % encoded again (%2520), the site answered 200 + HTML, and the HTML was saved as a .jpg and uploaded (broken thumbnails on Databases → News). Both collectors now decode-then-encode a path once and keep only a real JPEG/PNG/WebP/GIF (isPicture magic bytes) — an error page counts as failed. Fixing what's already up: delete the bad files, re-run the collector (pages are kept), then upload those files with the new **"Replace pictures the Hub already has"** tick on the News tools panel (sends every valid-named file chosen, regardless of missing). **Same day, pictures the collector CAN'T fetch** (dead rackcdn/CloudFront hosts, files the site has lost — ~345) go in each article's missingPictures in the JSON; the collect route stores a missing cover as NO cover (imagePath null) and flags a missing body picture missing: true inside bodyImages — the site page drops its <img>, the upload list skips it, and bodyImageKeys now keeps only keys of pictures still listed and not missing (no more "list changed → forget every key"). So a reload of the JSON is what forgets a wrongly-uploaded error page.

**Home page look (2026-09-24, Jordan's list, all done):** the top row runs to the page EDGES (search hard left, account hard right; the logo STAYS 172px — 250 was "too big", reverted the same hour; it was mathematically centred already, the unequal side blocks made it look off); the hero shows the picture at full strength with NOTHING over it (the old purple wash went; a left→right gradient was tried and Jordan said "remove the filter completely"); slide dots only with 2+ slides (a lone one looked like a stray mark); "Est. 1988" (was 1995 — the company-facts file); the stats band is the SAME FOUR red figures but COUNTED in app/(site)/site-stats.ts (lots with a hammer across ArchiveLot + WarehouseItem — the partial index ArchiveLot_sold_idx makes that cheap, NEEDS Run Migrations —, sales in the last 365 days from ArchiveSale, SiteDepartment count; cached 6 h in-process, 8 s timeouts — ⚠ set with $executeRawUnsafe and a constant: SET LOCAL statement_timeout = \${n} in a tagged template binds $1, which Postgres refuses in a SET, and every count silently showed "—" on the first deploy; a count not in yet shows "—", never a typed-in number like "180+ countries", which was also in the Why Vectis copy; "countries reached" can't be counted — BC holds no country — so the 4th stat is departments); HeroSlide.imageFocus top/center/bottom (**NEEDS Run Migrations**; nullable, no default; both reads tiered) = the Banner Manager's "Keep in frame" buttons; nav CAREERS → /careers (page existed), CONTACT US → new /contact (company facts only), SELL WITH US → /sell-with-us. **Banner editor (2026-09-24, Jordan: "a better banner editor so I can change font colours etc" → "as many as possible"):** each slide's look is JSON in HeroSlide.style (**NEEDS Run Migrations**; nullable) — small line text/show/colour, headline colour + size S/M/L, subtext colour, button colours, Register show/hide, an optional second button, words left/centre/right + top/middle/bottom + width, a per-slide shade 0–100% (the ONLY darkening there is now), seconds per slide. parseSlideStyle() in app/(site)/hero-style.ts is the ONE reader (defaults + validation — the site never renders a value it doesn't know). ⚠ app/(site)/hero-slide-view.tsx is the ONE rendering of a slide: the site's hero AND the manager's preview (a 1400×520 render scaled to fit) use it, so the preview IS the site — never fork the markup. A failed save is shown beside the buttons and names Run Migrations as the likely cause. **View Results cards:** every cover picture sits WHOLE (object-contain, white behind) in a fixed 220×220 SQUARE. The site's cover files are square, so whole = fills the square, exactly as vectis.co.uk's archive looks ("the Pokémon one is the perfect example"). ⚠ The ragged look was never contain — it was the box taking its height from the card's TEXT (a two-line title → a bigger picture); a blurred-backdrop version and a cropped (cover) version were also tried the same day and are not wanted.

**📄 PAGE EDITOR — Website → Pages (2026-09-24; Jordan: "a full blown page editor … I need it for every page").** Built on **Puck** (\`@puckeditor/core\` 0.23, MIT, runs inside the Hub — no outside service). Tables **SitePage** (slug PK, draft + published JSON, SEO title/description, who/when) and **SitePageVersion** (every publish, for History) — NEEDS Run Migrations. Admins only (pages, actions, upload); the help map gained \`adminOnly\` for it.
- **Nothing on the site changes until Publish.** Every built-in page asks \`publishedPage(slug)\` (app/(site)/page-editor/render.tsx) first and falls back to its built-in design; "Take the edited version off the site" brings the built-in design back (draft + history kept). Slugs: home, auctions, departments, sell-with-us, news-stories/news, how-to-bid, faq, contact, careers, terms, departments/<slug> (all 28), and NEW pages at /<slug> via app/(site)/[slug] — ⚠ RESERVED_SLUGS in page-editor/pages.ts lists every top-level route; add a new top-level route there or a new page could shadow it.
- ⚠ **ONE rendering per block** (page-editor/blocks.tsx): PURE — no hooks, no handlers — because the site draws them as server components through Puck's RSC Render (the fold-outs are <details>). Colours the author picks are inline styles; words take --pe-* CSS variables so a block dropped on a dark Section turns light by itself.
- ⚠ **Live blocks** (live-defs.ts: banner slides, upcoming, calendar, stats, specialisms, departments, a department's highlights / past auctions, news, the news listing, the sell form) render on the site with the SAME server components the built-in pages use — extracted to home-sections.tsx, auctions/calendar.tsx, departments/sections.tsx, news-stories/news/index-view.tsx, never copied. The editor can't read the database, so it shows each live block as the site's own render in a frame (/site-block — outside the (site) layout; height from a same-origin ResizeObserver).
- **Seeds** (seeds.ts): every built-in page as blocks, so the first edit starts from what the page looks like now; departments converted from the scraped HTML by seed-tools htmlToBlocks (tested on all 28: heading/text/video/button/picture each; the copy's H1 becomes H2 because the page header is the H1). ⚠ The seeds repeat the built-in pages' words — change one, change the other.
- **Pictures are stored as R2 KEYS** (site-pages/… via /api/website/page-upload) and shown through /api/website/picture, which signs a fresh address and redirects — a saved page never holds an address that expires. Allowed prefixes only (site-pages, hero-slides, news-photos, sale-photos, archive-photos, bc-photos).
- **Rich text** is Puck's TipTap field; the server renders it with @tiptap/html + happy-dom (both come with Puck). TipTap drops anything outside its schema — pictures and tables go in their own blocks.
- Editor: draft autosaves 1.5 s after a change; Publish = live + a version; More → History / Start again from the built-in design / Take off the site / Delete (new pages). The status line always says what happened. ⚠ editor.css paints Puck's fields light: the Hub's UNLAYERED dark form-control rule beats Puck's layered styles.
- NOT in the editor (yet): pages driven by one record — a sale's results, a lot, a news article, search, account, portal, live bidding.
- **Tested on the sandbox in Jordan's Chrome (2026-09-24, with his yes to publish there):** editor opens every page from its seed; select/edit/drag-drop; the draft autosaves and reopens; live blocks draw in their frames (banner slide, stats counting 1M+ / 140+ / 28, news, highlighted lots, past sales); Publish → the site draws the page through Puck's server render (rich text included); FAQ fold-outs open and the section tags jump; History lists the publish; "Take off" restores the built-in page; "Throw away the unpublished changes" clears the draft. Found and fixed in the same session: the More menu was clipped inside Puck's header (now drawn fixed outside it); long block names overlapped the panel heading (now one or two words); the Live label covered "See more" links (now small, bottom corner); the Pages list showed UTC times (⚠ server-side time formatting needs timeZone "Europe/London" — the Hub's usual pattern); "Your saved draft is open" straight after a publish (a draft identical to the live version now reads as the live version). The browser's confirm() boxes can't be clicked remotely — overriding window.confirm in the tab is how the clean-up was driven.

**Built so far:**
- 2026-09-23 — **Auction Calendar → View Results reads the Hub's sale databases** (Jordan: "we have the database now to populate the auction results"). app/(site)/auctions/results/data.ts is the one read-only data module: the list is ArchiveSale (sales that have happened — from the day AFTER the sale date, the Hub's 24-hour rule, since on the day itself it is still running; an undated sale once the site says finished — newest first, search on title/code, 24 a page, cover picture = our R2 copy signed for an hour else the site's S3 file; ⚠ the lot count is OUR count first — ArchiveLot by AuctionID / WarehouseItem numbered lots by code / BcLotWeb by siteSaleId — then the site's lots figure, which is 0 for a sale whose page alone was read, else "Results not in yet" — Jordan saw "Lot count not held" on a sale that ran that day); a sale's lots at /auctions/results/[siteId] (the website's sale number — the one id every sale has) come from ArchiveLot by auctionId for old-system sales, and for BC sales from WarehouseItem ⟕ BcLotWeb by the sale CODE (ArchiveSale.code, or read off any BcLotWeb row with that siteSaleId when the code column isn't filled), falling back to BcLotWeb.siteSaleId alone. Hammer 0/null = unsold; a banner shows lots / sold % / hammer total. ⚠ Every ArchiveSale read is tiered (with the 2026-09-22 code/hero columns, then without) because production may not have had Run Migrations. ⚠ The results tab NO LONGER lists the Hub's own past CatalogueAuctions (they'd duplicate the BC sales) — a Hub sale still has /auctions/[code]. The calendar sidebar takes {date, href} entries. Photos are plain img tags on signed/S3 URLs (never next/image — that would need a shared next.config change).
- 2026-09-23 — **Lot page** /auctions/results/[siteId]/lot/[lotId] (Jordan: "I can't click on the lots, it should open an item page … don't copy the design though"): its own layout — photo large with the Hub zoom viewer, the result (hammer or Unsold) up front, estimate, full description, key information, previous/next lot by lot order. The grid card is ONE link (no zoom button inside a link). The lot id in the address is ArchiveLot.id for ABC sales and the BC unique id (R008728-194) for BC ones — getResultLot() in data.ts checks the lot belongs to that sale. Headline = first 120 chars cut at a word, never at a full stop (the Hub title rule).
- 2026-09-23 — **News & Stories** (Jordan: "how can we scrape the news articles off the site along with the images?" → Databases → News tab, Node collector with pictures). ⚠ The site's news is a **Joomla blog with a JSON feed**: POST index.php?option=com_blog&format=json&task=blog.getArticles (form body: start, selected_category, search, articles_per_page ≤ 100) → {status, articles, total_articles, featured_articles}; task=blog.getCategories. Each article carries its FULL text (HTML), catid, tags, dates, sef_link and images (a JSON string; image_intro = images/2026/09/x.jpg#joomlaImage://… — the part before # is a plain file on the site, fetchable from an office PC, ~430 KB). 1,337 articles = 14 requests. **Collector** scripts/collect-news.mjs <folder> → vectis-news.json (12.5 MB — one file, under the 20 MB body limit; split if it ever grows) + pictures/<id>.<ext> (573 MB) + log; first run 2026-09-23 on Jordan's PC into Downloads\\vectis-news: 1,337 articles, 1,336 pictures, 0 inline pictures in the text. **Hub**: table SiteNewsArticle (NEEDS Run Migrations; alias is an index not unique — Joomla only enforces it per category), /databases/news (tiles, filters, cards, "On the test site →"), 📥 Update the news = copy instructions for Claude → load the JSON (POST /api/databases/news/collect, raw upsert 100 a chunk, site text overwrites ours, our picture kept unless the site's file changed) → choose the pictures folder (browser → presigned PUT → news-photos/<id>.<ext>, registered by /api/databases/news/picture, HEAD-checked, idempotent; live "12 of 400" + Stop). The server can't fetch the pictures itself (same host as the refused feed). **Site**: /news-stories/news (header with search + category, featured strip, 24 a page) and /news-stories/news/[alias] (site HTML cleaned: no scripts/on-handlers, relative images/ made absolute; .news-body styles in site.css; 3 more from the same category); nav NEWS & STORIES points there. Data map + help map entries added.
  - ⚠⚠ **The feed's fulltext is FLATTENED** (found 2026-09-23 when Jordan compared an article: "hasn't got the images inside the articles or the format correct, missing spaces, emojis"): tags stripped, paragraphs collapsed to runs of 3+ spaces, emojis dropped (the title keeps them), and image_fulltext is empty — the article's own picture (a square social image, e.g. images/2026/09/22/lot-595.jpg) exists ONLY on the article PAGE as an SP Page Builder lazy image (src = placeholder, real file in data-large); emojis there are Facebook emoji img tags with the emoji in alt. So the collector reads every article's page too (builderBlock = the div id="sp-page-builder" by counting divs → cleanBlock: scripts/styles out, emoji imgs → alt, lazy imgs → data-large, only href/src/alt/width/height kept, empty p gone) and downloads the pictures inside as <id>-<n>.<ext>; re-runs re-read a page only when the feed's modified changed. Columns bodyHtml, bodyImages [{path,file}], bodyImageKeys (NEEDS Run Migrations again on the sandbox). The article page renders bodyHtml with our R2 copy per picture (news-photos/<file> in bodyImageKeys → signed, else the site's file), shows the cover only when the body has no picture, and for old loads splits the flattened text on runs of spaces into paragraphs. The picture route appends inside-pictures with array_append … AND NOT (key = ANY(...)); the loader resets bodyImageKeys when bodyImages changes. ⚠ The collector writes PARTS — vectis-news-1.json, -2.json … (250 articles each, ~15 MB in all) — because one file with the page text would pass the 20 MB request limit; the load step takes all the parts at once (one request each). ⚠ The older articles (2017–2023, 746 of 1,337) are NOT page-builder pages — their text is in div.com-content-article__body; builderBlock tries both markers. One old article carried its picture pasted in as an 880 KB base64 img — written out as pictures/<id>-<n>.<ext> under a stand-in path inline/<file>. ⚠ **346 pictures inside older articles are on DEAD hosts** — the old site's Rackspace CDN (…cf3.rackcdn.com, 404) and its old CloudFront image store (d25awfn7k3cact.cloudfront.net, answers 202 + empty to everything, even from the office) — gone everywhere, the live site can't show them either; the article page drops an off-site or inline picture that hasn't been uploaded rather than show it broken. Third run on 2026-09-23: 1,337 articles all with page text, 1,336 covers + 1,888 inside pictures, 2,878 files / **3.4 GB** in pictures (page-builder images are ~1 MB each); the upload runs 4 files at a time.
- 2026-09-23 — **Departments** (Jordan: "what about the department pages?" → scrape what's there now, an editor "eventually"; he showed the site's DEPARTMENTS dropdown — two columns, 28 departments, "View all departments"). A department page on the live site = SP Page Builder: banner (data-large), "SELL … AT AUCTION" copy (h1, paragraphs, brand list, contact, "Why use Vectis?" bullets), a "Sell your collection with us" box, **Highlighted Lots** (16 hand-picked past lots: picture, title, "Hammer Price: £…", date · category · brand, link with ?el=<siteLotId>), a Latest-news module (3 stories), and a **Past Auctions** module OUTSIDE the builder block (/bidding/<code>-…-<siteId> links). The index is 28 tiles — ⚠ a tile picture's own link can be a stale template address (Star Wars' picture links to /departments/antiquities; the index also carries hidden template tiles like "Modern British 20th Century Art"), so tiles are matched to the menu by NAME. Menu slugs differ from tile slugs (star-wars-auctions vs star-wars). scripts/collect-departments.mjs <folder> → vectis-departments.json + pictures/dept-<slug>-hero|tile|<n>.<ext> (same folder as the news: 28 departments, 400 highlighted lots, 447 pictures on the first run). Table SiteDepartment (NEEDS Run Migrations): copy, highlights JSON + highlightKeys, newsAliases → newsCategory seeded at load from the stories the page showed (most common category), saleKeywords seeded from the name (split on &/and/,), siteSaleIds = the Past Auctions listed. ⚠ newsCategory and saleKeywords are only seeded when blank — a later edit survives reloads. ⚠ Jordan: "why would I do it through the news, they are totally separate" → **its own page /databases/departments** (tiles, one card per department with news category + sale words, 📥 Update the departments = copy instructions / load vectis-departments.json / upload its pictures — the collect route is shared and branches on kind "departments"; the picture routes take dept- names: -hero → heroKey, -tile → tileKey, -<n> → highlightKeys). The Downloads are split too: Downloads\\vectis-departments (json + 474 pictures) beside Downloads\\vectis-news; the collector's instructions name that folder. The News page carries nothing about departments. ⚠ Jordan (2026-09-23, Action Figures page): "the format is wrong and it's missing images like the prices realised image" — the section's SECOND column is the "Sell Your Toys" side box: a Vimeo tour video (iframe src=player.vimeo.com/…), "Sell your collection with us" text, GET STARTED (→ /valuations… on the site, rewritten to /sell-with-us at render) and the "PRICES ACHIEVED BY VECTIS" picture. The collector now splits section 0 by its top-level sppb-col-* columns (columnsOf): column 1 → copyHtml (its own h1, bold/italic runs, lists, pictures kept), the rest → sideHtml; Vimeo/YouTube iframes kept as bare iframe src, other frames dropped; pictures inside either become extraImages [{path, file: dept-<slug>-x<n>.<ext>}] with extraImageKeys (28 side boxes, 28 videos, 27 pictures on the re-run). The site page renders the copy as-is (heading no longer forced upper-case) and the side box in a navy .dept-side block (site.css: 16:9 iframe, full-width picture). Site: the nav's DEPARTMENTS dropdown reads the table (falls back to the old hardcoded list → /auctions?type=), /departments tiles, /departments/[slug] = banner, copy (.news-body), sell-with-us box + contact, latest news (category else keywords on titles), highlighted lots (linked to OUR results lot page when siteLotId is in ArchiveLot/BcLotWeb, else vectis.co.uk), past auctions (listed ids ∪ keyword title matches, happened = day after). cleanHtml moved to news/shared.tsx and is shared.
- 2026-09-23 — **Below the lot** (Jordan: "we need something similar to this below the lot as well", from the live site's lot page): the FULL description moved down there (one place only) with Vectis's own "THIS IS NOT A WARRANTY… By bidding on any lot, you agree" disclaimer; two native details fold-outs — **Fees & VAT** (his exact wording from the live site: 22.5% + VAT = 27%, payment methods, 32.94% via the-saleroom, no extra charge on Vectis Live, description-over-image rule, +44 (0) 1642 750616 / admin@vectis.co.uk) and **Shipping and postage** (Jordan pasted the live site's wording the same day: UK Small £15.95 / Medium £20.95 / Large £25.95 + VAT with per-extra-lot charges and a worked example, Parcel Force zones 6–8 and zone 9 tables, rest of world = quote, US customs 10–50%, dispatch@vectis.co.uk, no combining across sale dates — the live site's "currently dispatching week commencing…" status line was left OUT because it goes stale weekly); a View FAQs button (→ /faq); and a "Got one to sell?" card (the sale's cover picture, 98% claim, → /sell-with-us).`,
  },
  {
...
- Filters: sources, sale, year range, hammer range, "estimate around £X" (lots whose low-high estimate COVERS the figure; replaced from/to boxes 2026-09-11 - Jordan: a from/to pair made no sense against a low-high estimate), sold/unsold, category/subcategory, "leave out lots mentioning", exact phrase, photo only. Sorts: newest, oldest, hammer and estimate both ways.
...
- WHOLE NUMBERS (Jordan, 2026-09-11 - "Class 37" was bringing up every Class 373): a word that starts or ends with a digit may not have more digits glued on at that end - "37" finds Class 37, 37/5, No.37, never 373/3714/37417; letters may touch ("3514" still finds R3514). An "Exact numbers" tick in the filters (an i button beside it and beside "Exact phrase" explains each, tapped not hovered), ON by default (his choice: a toggle, not typing quotes), with a note under the results. ILIKE first pass + a ~* pattern for the edges, measured no slower (class 37: 2.4 s, 13,414 -> 3,406 lots). ID fields stay a plain contains.
- EXACT WORDS (same day) - OFF by default: a word may not sit inside a longer word, though its own plural still counts ("bus" finds buses, never business/omnibus). One edgePattern() in the route: Exact words governs LETTER edges, Exact numbers DIGIT edges. Three ticks, each with an i: Exact phrase, Exact words, Exact numbers.
...
- ⚠⚠ **\`relpersistence\` is selected as \`::text\`** (25 Sept). Postgres's one-byte \"char\" type can't be read by the Prisma pg adapter (\`Failed to deserialize column of type 'char'\`), so from the 22 Sept rewrite every nightly backup, every manual backup and every restore died on \`describeTables()\` before copying a thing. Cast any pg_catalog \"char\" column the same way.
...
**Doing it again.** BcLotWeb.siteSaleId (**NEEDS Run Migrations**; the write falls back cleanly without it) records which website sale number each lot came from, so the page reads back "collected up to sale N". Re-collecting old sales is safe — every column is COALESCEd, so a second pass only fills blanks.

⚠⚠ **The website's "finished" is NOT "held" (2026-09-25).** The feed marks every lot isFinished on sales still WEEKS away, and sale numbers are given when a sale is LISTED — so the 17 Sept run took F115–F127 early (descriptions, no hammer), max(siteSaleId) said 1566, the next run started at 1567 (nothing there) and yesterday's F117 was never picked up. Both collectors now write a sale only when its lots are finished AND its page's date is before today in London, and end with "NEXT TIME START FROM SALE N". The page's start point is the FIRST sale still waiting (an ArchiveSale dated today or later, or a sale near the top with no hammer at all), not max+1. The copied instructions name a folder per run (vectis-bc-lots-<from>) because the files are always -01, -02… and overwrite. The table has a **Photo in Hub** column (✓ = photoKey, "website only" = sitePhoto not copied yet). The collector's **--upcoming** flag also takes catalogues of sales not yet held (lots written, sale marked finished:false with its date; loading again later replaces the description — newer wins). The page LISTS upcoming sales too, with a blue **Upcoming** badge (London date) and a "Held / Sold / Unsold / Upcoming" filter; ⚠ the summary tiles stay on HELD sales only, upcoming get their own line.
...
- Customer photo request link: Submission.photoUploadToken (String? @unique). Collections/admin see "Photo Request Link" sidebar card. Link /submit/[token] — public step-by-step wizard (Take a Photo / Record a Video / Choose from Gallery), no size limits, accepts any image OR VIDEO type. Videos since 2026-09-14: uploaded straight to R2 with a live percentage; the key always ends in an extension, which is the ONLY way the Hub tells a video from a photo (lib/media.ts — Item.imageUrls holds both); the submission page and the cataloguer valuation page play them in a pop-up (components/video-modal.tsx) that also offers the file itself, because iPhone HEVC .mov may not play in Chrome. A refused upload now shows Failed — the old fetch showed a green tick whatever the storage answered. Odd file types (2026-09-15, "our customers are nightmares for sending all sorts of random file types"): the prebuilt sharp can't decode iPhone HEIC (it reads the size, not the pixels — HEVC isn't built in). lib/media-convert.ts makes a JPEG copy beside the original (key + .jpg, max 2400px) for HEIC (heic-decode, libheif WASM), TIFF (sharp) and camera RAW (the camera's own embedded JPEG preview), one at a time; /api/media/view redirects to it. lib/video-convert.ts (ffmpeg-static) checks each video once and makes an H.264 key + .mp4 for anything browsers can't play (HEVC, AVI, WMV…), one at a time at the lowest CPU priority; /api/media/video says which to play. getSignedViewUrl (lib/r2.ts) is the displayable link — getSignedImageUrl stays the original. Used on the submission page, the valuation page, Accounts, the Job Board, the First Aid public page and Induction slides; First Aid and Induction uploads are stored as JPEG. PDFs and other files keep their own type (they used to be saved as JPEG). The public save route converts in the background as soon as a customer sends. Download all (2026-09-15): the submission page's Attachments row has a Download all button — pick a folder (the File System Access API, Chrome and Edge on a PC) and every file is saved there one at a time, with a count and a Stop. Photos a PC can't open come as their JPEG copy and videos as their playable copy; /api/media/file streams them THROUGH the Hub because the page can't read R2's links itself. An existing file is never overwritten ("IMG_1 (2).jpg"). Safari, Firefox and the iPads get one download per file instead. Both public pages show Vectis logo.
