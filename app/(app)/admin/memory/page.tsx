...
⚠ THE ADMIN CENTRE COURSE LIVES IN lib/training-admin-centre.ts, not in training-seed.ts (which re-exports it). 35 slides and 23 scenarios as of 2026-08-19. A whole section of the deck is about NAMES, because that is what confuses people: the Hub stores the person who typed the lot ("Jack Collings"), while BC stores a short code (JC / JO / KS, blank on tens of thousands of lines), a catalogued-by Windows username, or a created-by username - resolved by bcPersonName in lib/cataloguer-directory.ts, code first, then username matched to a directory EMAIL local-part, then title-cased. The BC_NAME scenario only ever picks a record bcPersonName resolves to a real name (tests for a space), because setting a task whose answer is a tidied-up username would teach the tidied username as correct.
