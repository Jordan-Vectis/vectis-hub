...
**PDF export (2026-07-30).** "⬇ Export PDF" on the card header + a "+ totes" variant (\`?totes=1\`) that also lists the 10 totes behind each month. Builder \`lib/bc-tote-dates-pdf.ts\` — pdf-lib, A4 portrait, \`embedVectisLogo\`, headline boxes (categories / furthest behind / most up to date), same red-amber lag colouring as the screen, footnotes explaining the metric + date provenance. Route is gated on **MANAGER_PORTAL** app access (the JSON route stays session-only) and returns a readable 503 when BC isn't connected. ⚠ **Hidden categories are excluded from the PDF too** (Jordan asked) — the hidden set is passed into \`computeBcToteDates\` exactly as the screen route does it.
