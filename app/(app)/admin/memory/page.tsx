...
- **API \`GET /api/catalogue/end-of-day\`** (CATALOGUING app access): all \`CatalogueLot\`s in **non-complete sales** (\`?includeComplete=1\` widens), checked against the synced \`WarehouseItem\` data, case-insensitive via the variants trick (Prisma \`in\` is case-sensitive). ⚠ Deliberately NOT \`CatalogueLot.addedToBC\` — same reasoning (and same measurement) as the Admin Centre. ⚠⚠ **A lot WITH a barcode is judged by its BARCODE ALONE (fixed 2026-08-07)** — the \`uniqueId\` fallback applies only to barcode-less lots. It used to be an OR across both fields, and **legacy Hub-minted provisional IDs ({receipt}-N, pre-2026-08-06) collide with BC's own numbering for OTHER items** (F121276 carried R009332-1 = BC's F114104), so **292 pending lots across 10 sales were silently counted "already in BC" and kept off the sheet** (F121 read 132 of 184). Never re-add the OR; the stale minted uids stay on the lots until 🔗 BC Match overwrites them.
