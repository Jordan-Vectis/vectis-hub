...
2. **Cataloguing** — iPads, lot wizard: scan/type the tote → vendor + receipt pulled from BC tote data; photos; key points; F-number barcode label on the item. The Hub mints a **provisional** unique ID from the receipt (\`R000016-413\`, advisory-lock + MAX scheme).
...
5. **Linking back — the 🔗 BC Match button** (Auction Manager sale page → "BC Match & Import" modal, \`auction-tabs.tsx\`): upload the BC **Lines export**; it matches rows to Hub lots **by barcode**, compares receipts, and where the receipt agrees imports **BC's UniqueID onto the Hub lot** (\`bulkAssignUniqueIds\`). From here the Hub's \`receiptUniqueId\` = BC's — the provisional Hub ID is replaced.
...
- **\`receiptUniqueId\` is provisional before step 5, authoritative after.** A Hub↔BC unique-ID comparison is only meaningful AFTER 🔗 BC Match has run for those lots. Barcode is the stable identifier through the whole life (which is why Admin Centre, End of Day and BC Import Check all match on barcode).
