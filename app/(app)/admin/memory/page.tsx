...
**Metric (Jordan's final):** "Using totes from" = the **MEDIAN goods-received month of the newest 10 CONSIGNMENTS catalogued in each category** ("Mar 2026"). Group catalogued \`WarehouseItem\` by \`receiptNo\` (a receipt ≈ a tote/consignment — items don't carry a tote no, \`EVA_ArticleToteNo\` empty on ~all), rank by \`MAX(cataloguedAt)\` desc, take top 10, median their \`MIN(goodsReceivedDate)\` (guard >= 1990). ⚠ Median NOT trimmed-mean (a small sample can't be trimmed; median is robust at any n). Same dating path departments-view.tsx calls "the path that works for the real data" (receiptNo → goodsReceivedDate).

**⚠⚠ WHY NOT the live tote feed (the wall that ate ~8 rounds).** The obvious source — \`Receipt_Totes_Excel\` filtered to \`PTE_Benched\` — CANNOT work: **that's BC's ACTIVE-totes feed; a tote drops out once finished/shelved.** GAMING had **1,729 catalogued items but only 5 benched totes in the feed** — no fetch pattern fixes that, the finished totes aren't in the feed at all (the fuller view Jordan sees in the BC client isn't on this OData endpoint). So the table now uses our COMPLETE item-level \`WarehouseItem\` data, DB-only, no live BC, can't run dry. (\`goodsReceivedDate\` = when the stock arrived = the "how far behind" date; it's reliably populated when joined by receiptNo.)

**⚠ Dead ends, do NOT retry:** the live \`Receipt_Totes_Excel\`/\`PTE_Benched\` feed (active-only, missing finished totes — see above); keying by our departments; "frontier"/newest-catalogued MAX (cataloguing is OUT OF ORDER → nonsense); filtering WarehouseTote.catalogued (all-false + it lacks finished totes' dates too); linking catalogued items by toteNo (items lose it); WarehouseContainer (internal warehouse, different system). All date parsing guards >= 1990 (BC empty date = 0001-01-01).
