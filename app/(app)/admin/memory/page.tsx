...
**How the source gets in:** an **admin-only** zip upload (zip the vendor's "Source" folder → the server unzips with jszip and stores text files in \`BcSourceFile\`, replace-all in one transaction; single-root zips are auto-unwrapped). ⚠ **The source lives in the DB, deliberately NOT in the git repo** — it is Evo-soft's proprietary code and must stay out of GitHub. Viewing needs only the IT Tools app permission; replacing the source needs ADMIN.
...
- **chat** — "Ask the code": keyword-scores every stored file against the question (same retrieval pattern as IT Help's ask route; path matches weighted ×4), feeds the top ~25 files (≤300k chars) to Gemini (slot \`bc_source_chat\`), and answers **citing file paths**; cited files are clickable chips that open in the browser.
