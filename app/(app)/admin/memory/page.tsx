...
A BARCODE IS NOT UNIQUE IN BC. Measured on production: 2,039 barcodes appear under MORE THAN ONE RECEIPT. F109050 exists as R009300-1 (customer C209142 - our real, described, catalogued lot) AND as R008537-51 (customer C223610 - an empty A995 placeholder). Two consequences, both fixed: (1) pickBcFor() - matching a Hub lot to a BC row on barcode alone used a Map, so WHICHEVER ROW WAS WRITTEN LAST WON and a lot could display A DIFFERENT CUSTOMER's BC record; the RECEIPT now decides, the customer is the second check, and an ambiguous barcode attaches to NOTHING rather than guessing. (2) Leftover BC rows are looked up in the Hub BY BARCODE ACROSS ALL CUSTOMERS and ours is used when we have it - otherwise a barcode we had plainly catalogued rendered as "Waiting to be catalogued / No description yet" purely because BC had filed it under someone else. Measured on C223610: 110 of its 142 BC rows are answered from the Hub instead.
