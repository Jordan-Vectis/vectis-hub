              ...
              <tbody>
                {data.categories.map((c: any) => (
                  <tr key={c.category} className="border-b border-gray-200 dark:border-gray-800 hover:bg-gray-800/50">
                    ...
                    <td className="px-4 py-2 text-right font-mono text-gray-600 dark:text-gray-500">{fmt(c.totalEstimateMid)}</td>
      ...
      <tbody>
        {lots.map((l, i) => (
          <tr key={l.uniqueId} className="border-b border-gray-200 dark:border-gray-800 hover:bg-gray-800/50">
            ...
            <td className="px-4 py-2 text-gray-600 dark:text-gray-400">{l.category ?? "—"}</td>
            <td className="px-4 py-2 font-semibold text-green-400 whitespace-nowrap">{fmt(l.hammerPrice)}</td>
            <td className="px-4 py-2 text-xs whitespace-nowrap">
              <div className="text-gray-600 dark:text-gray-300">{l.auctionName ?? l.auctionCode ?? "—"}</div>
              {l.auctionDate && (
