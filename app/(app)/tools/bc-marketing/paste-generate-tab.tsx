...
export default function PasteGenerateTab() {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 space-y-6">
      ...
      <div className="bg-gray-900 border border-gray-700 rounded-xl p-5 space-y-4">
        <div>
          <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Paste lot details</label>
          <textarea
            value={pasted}
            ...
            rows={14}
            className="w-full bg-gray-800 border border-gray-600 rounded-lg px-3 py-2 text-sm text-gray-200 font-mono focus:outline-none focus:border-pink-500 resize-y"
          />
          <p className="text-[11px] text-gray-500 mt-1">{pasted.length.toLocaleString()} characters · max 200,000</p>
        </div>

        <div>
          <label className="block text-xs text-gray-400 mb-1">Optional context for the AI</label>
          <input
            type="text"
            value={contextNote}
        ...
        <div className="flex items-center gap-3 flex-wrap pt-2">
          ...
          <div className="flex items-center gap-1.5">
            ...
            {savedDefault === modelId ? (
              ...
              <button onClick={setAsDefault} className="text-xs text-gray-400 hover:text-pink-400 px-2 py-1 rounded">Set as default</button>
            )}
          </div>
        ...
        <div className="bg-gray-900 border border-gray-700 rounded-xl overflow-hidden">
          <div className="px-5 py-3 border-b border-gray-700 flex items-center justify-between gap-3 flex-wrap">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={generate} disabled={loading} className="text-xs bg-pink-700 hover:bg-pink-600 disabled:opacity-50 text-white px-3 py-1.5 rounded-lg">Regenerate</button>
            </div>
          </div>
