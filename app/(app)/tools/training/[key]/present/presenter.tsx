...
// A training deck on a room screen, run by whoever is doing the training.
//
// Same mechanics as the induction presenter, and for the same reasons — the fit-to-screen
// maths in particular. It covers the Hub shell (fixed inset-0) rather than living on a separate
// top-level route: the app's own nav down the side of a projected slide looks like a mistake,
// but the page still sits inside (app) so the login gate applies exactly once.
...
export default function TrainingPresenter({
  moduleKey, title, slides,
}: {
  moduleKey: string
  title: string
  ...
  const back = useCallback(() => router.push(`/tools/training/${moduleKey}`), [router, moduleKey])
  const next = useCallback(() => setI(v => Math.min(v + 1, total - 1)), [total])
  const prev = useCallback(() => setI(v => Math.max(v - 1, 0)), [])

  // ── Fit the slide to the screen ────────────────────────────────────────────
  // ⚠ Letting a long slide scroll means the presenter is scrolling mid-sentence and the back of
  // the room reads half a bullet list. offsetHeight reports the LAYOUT box, which a CSS
  // transform does not affect, so measuring while scaled is stable and cannot feed back into
  // itself.
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      // overflow-hidden, so there is no scrollbar to hint at it. Say so rather than letting
      // three bullets vanish in front of a room.
  ...
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      ...
      else if (e.key === "Escape") { if (document.fullscreenElement) document.exitFullscreen().catch(() => {}); else back() }
      else if (e.key.toLowerCase() === "n") setShowNotes(v => !v)
      else if (e.key.toLowerCase() === "f") toggleFullscreen()
    }
    window.addEventListener("keydown", onKey)
    return () => window.removeEventListener("keydown", onKey)
  }, [next, prev, total, back])

  // The controls fade out while nobody is touching anything, so the room sees the slide and not
  // a toolbar. Any movement brings them back.
    ...
    return (
      <div className="fixed inset-0 z-[150] bg-white dark:bg-[#0b0b0d] flex items-center justify-center p-8 text-center">
        <div>
          ...
          <p className="text-gray-500 mt-2">Add some on the Edit tab, or tick an existing one back into the deck.</p>
          <button type="button" onClick={back}
            className="mt-6 px-6 py-3 min-h-[44px] rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-semibold">
            Back to {title}
          </button>
        </div>
      </div>
  ...
  return (
    <div className="fixed inset-0 z-[150] bg-white dark:bg-[#0b0b0d] flex flex-col overflow-hidden">
      {/* A soft wash in the corners, so a slide is not white type on a black rectangle */}
      <div aria-hidden className="pointer-events-none absolute inset-0 opacity-70"
        style={{ background: "radial-gradient(60rem 40rem at 12% -10%, rgba(99,102,241,0.14), transparent 60%), radial-gradient(50rem 35rem at 100% 110%, rgba(99,102,241,0.09), transparent 60%)" }} />

      {/* Controls — fade out while presenting, come back on any movement */}
      {/* ⚠ Two layers, and both halves matter. The BUTTONS go pointer-events-none while faded,
          or an invisible bar still takes the tap — on a tablet, touching the top-left to bring
          the controls back landed on ✕ Finish. But making them inert alone means the first tap
          does nothing at all and the user taps twice wondering why. So the WRAPPER stays live
          and its only job, while faded, is to bring the controls back. */}
      <div className="relative z-10 shrink-0"
        ...
        <div className={`flex items-center gap-3 px-4 py-2 transition-opacity duration-500 ${chrome ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
          ...
          <div className="flex-1 h-1.5 rounded-full bg-gray-200 dark:bg-white/10 overflow-hidden">
            <div className="h-full bg-gradient-to-r from-indigo-400 to-indigo-600 transition-all duration-300" style={{ width: `${((i + 1) / total) * 100}%` }} />
          </div>
          {clipped && (
          ...
          {slide.notes && (
            <button type="button" onClick={() => setShowNotes(v => !v)}
              className={`px-3 py-2 min-h-[44px] rounded-lg text-sm font-semibold ${showNotes ? "bg-indigo-100 dark:bg-indigo-500/20 text-indigo-700 dark:text-indigo-300" : "text-gray-500 hover:text-gray-900 dark:hover:text-white"}`}>
              🗒 Notes
            </button>
          )}
          ...
          <button type="button" onClick={next} disabled={i >= total - 1}
            className="px-5 py-2 min-h-[44px] rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-semibold disabled:opacity-30">Next →</button>
        </div>
      </div>
      ...
      <div ref={stageRef} className="relative z-10 flex-1 min-h-0 overflow-hidden">
        ...
        >
          <div ref={contentRef} className="max-w-[1400px] w-full mx-auto py-6 xl:py-10 [&_a]:pointer-events-auto [&_iframe]:pointer-events-auto [&_img]:pointer-events-auto">
            <TrainingSlideView slide={slide} big />
          </div>
        </div>
      </div>
      ...
      {showNotes && slide.notes && (
        <div className="relative z-10 shrink-0 border-t-4 border-indigo-500 bg-indigo-50 dark:bg-indigo-500/10 px-6 py-4 max-h-[30vh] overflow-y-auto">
          <p className="text-xs font-bold uppercase tracking-wide text-indigo-700 dark:text-indigo-400 mb-1">Presenter note — not part of the slide</p>
          <p className="text-sm text-indigo-900 dark:text-indigo-200 whitespace-pre-line leading-relaxed">{slide.notes}</p>
        </div>
      )}
