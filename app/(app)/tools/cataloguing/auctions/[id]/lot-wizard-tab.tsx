  ...
  return (
    ...
    <div className="flex flex-col h-full"
      ...
      <div className="flex-1 min-h-0 overflow-y-auto">
        {/* Desktop only (desk: = a mouse AND a wide screen — Jordan, 2026-09-11): the screen split
            50/50 — the step and its banners filling the left half, "This lot so far" the right
            ("it should be more 50/50 split between the 2 boxes"). On a tablet these two wrappers do
            nothing at all. */}
        <div className="desk:grid desk:grid-cols-2 desk:items-start desk:gap-8">
        <div className="min-w-0">
        {step === 1 && (
          <div className="max-w-lg desk:max-w-none space-y-4">
        ...
        {step === 4 && (
          <div className="max-w-lg desk:max-w-none space-y-5">
            <div>
      ...
      {/* ⚠⚠ THE BANNERS ARE AT THE BOTTOM, BELOW THE STEP (Jordan, 2026-09-09: "these warning
          ...
          ⚠ `max-w-lg desk:max-w-none` on each, matching every step's own container — Jordan, same day: "why does it
          ...
          beside a form. */}
      {staleDeploy && (
        <div className="mt-4 max-w-lg desk:max-w-none rounded-xl border border-amber-500 bg-amber-500/10 px-4 py-3">
      ...
      {rememberFailed && !staleDeploy && !hidden.remember && (
        <div className="mt-4 max-w-lg desk:max-w-none flex items-start gap-2 rounded-xl border border-amber-500/60 bg-amber-500/10 px-4 py-2.5">
      ...
      {step > 1 && (vendor || tote || receipt) && (() => {
        ...
        return (
          <div className={`flex flex-wrap items-center gap-x-4 gap-y-2 mt-4 max-w-lg desk:max-w-none rounded-xl border px-4 ${tablet ? "py-3" : "py-2.5"} ${
        ...
        {/* ── This lot so far — DESKTOP ONLY (Jordan, 2026-09-11) ─────────────────────────────
            Uses the right half of the screen: everything entered so far, filled in as you go.
            ⚠ DISPLAY ONLY — nothing in it can be clicked (Jordan, same day: "it's more likely they
            will click it by accident, it just needs to be there for show"). Moving between steps is
            Back and Next only. Hidden on the tablets, where there is no room for it. */}
        {(() => {
          ...
          return (
            <aside className="hidden desk:block desk:sticky desk:top-0 min-w-0 rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-[#1C1C1E] p-4">
                ...
                {soFar.map(row => {
                  const body = (
                    <>
                      ...
                      {/* ⚠ Cut to 3 lines with "…" — a long description must never make the page scroll
                          (Jordan: "if they type a lot it should just go ..."). ⚠ No `block` class here:
                          it overrides line-clamp's own display and the clamp silently stops working. */}
                      <span title={row.value || undefined}
                        className={`whitespace-pre-line break-words line-clamp-3 text-sm ${row.value ? "text-gray-900 dark:text-gray-100" : "text-gray-400 dark:text-gray-600"}`}>
