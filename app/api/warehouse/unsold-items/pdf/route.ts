...
// GET /api/warehouse/unsold-items/pdf?aisles=A50,A51
//
// Server-side PDF generator. Same layout as Collections Due — Vectis-branded
// per-aisle pages, picker tickbox column — but the third info column is the
// vendor name (so the picker can see who consigned each item) and the filter
// is "Hammer Price = 0" rather than "has collection docket".
...
export async function GET(req: NextRequest) {
  try {
    ...
    // Parallel per-aisle queries — same approach as Collections Due
...
function renderAisleReport(
  ...
  for (const it of items) {
    ...
    // Show vendor — name preferred, fall back to vendorNo
    const vendor = it.vendorName || it.vendorNo
    page.drawText(safeAscii(vendor),      { x: COL.vendor.x,   y: y - 8, size: 8, font: fonts.helv, color: black })
    // Tickbox
    page.drawRectangle({
...
function drawTableHeader(page: PDFPage, y: number, font: PDFFont) {
  ...
  page.drawText("VENDOR",      { x: COL.vendor.x,      y: y - 8, size: 8, font, color: black })
