...
export async function POST(req: NextRequest) {
  try {
    ...
    const [fights, rosterRows] = await Promise.all([
      ...
      prisma.mcocChampion.findMany({ where: { ownerId: session.user.id }, select: { name: true, stars: true, rank: true } }),
    ])
    ...
    const rosterList = rosterRows
      .map((c) => `- ${c.name}${c.stars ? ` (${c.stars}★${c.rank ? ` R${c.rank}` : ""})` : ""}`)
      .join("\n")
...
1. TWO or THREE different 3-champion attack teams from the roster that could each clear this whole path. Make them genuinely different (different classes / playstyles), each with a short name and a one-line game plan.
2. For EACH fight, the 2–3 BEST attackers from the roster for that specific defender + its nodes (best first), each with a short note on how.
