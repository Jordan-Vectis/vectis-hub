  ...
  try {
    ...
    const [roster, profiles] = await Promise.all([
      ...
      prisma.mcocChampionProfile.findMany({
        select: { name: true, nameNorm: true, class: true, tags: true, profileAt: true },
        orderBy: { name: "asc" },
      }),
    ])
    ...
    // Coverage = tags on the rank 4+ champs. A champ that contributes nothing is
    // reported, so a "missing" tag can't be an artefact of incomplete data
    // without Jordan being told. The two ways that happens need DIFFERENT
    // fixes, so they are reported separately — saying "run Build profiles" for
    // a name mismatch sends him off on a ~250-champ rebuild that cannot help:
    //   unmatched  = no Champion DB row with that name at all. The two names are
    //                AI-generated by different prompts (roster photo scan vs the
    //                grounded catalog), so variants drift: "Spider-Man" here vs
    //                "Spider-Man (Classic)" there.
    //   unprofiled = row exists, profile never built. Build profiles DOES fix it.
    ...
    for (const c of high) {
      ...
      if (!p) {
        // SUGGEST only — never auto-match. normChampName is deliberately exact
        // because loose matching produced wrong-champ matches before, and a
        // prefix is genuinely ambiguous (Cyclops has several in-game variants).
        const suggestions = profiles
          .filter((x) => x.nameNorm.startsWith(key) || key.startsWith(x.nameNorm))
          .map((x) => x.name)
          .slice(0, 4)
        unmatched.push({ name: c.name, suggestions })
        continue
      }
      if (!p.profileAt) { unprofiled.push(c.name); continue }
