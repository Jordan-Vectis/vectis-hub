...
//
// The site's text is the authority and overwrites ours on every load. Our own copy of the cover
// picture (imageKey, uploaded separately into R2) is KEPT unless the site's picture file has
// changed, when it is cleared so the new one is asked for again.
type Incoming = {
...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const file of files) {
      ...
      for (let i = 0; i < rows.length; i += CHUNK) {
        ...
        const values = Prisma.join(chunk.map(r => Prisma.sql`(
          ...
          ${r.publishedAt}::timestamp, ${r.modifiedAt}::timestamp, ${r.imagePath}, ${r.imageAlt}, now()
        )`))
        written += await prisma.$executeRaw`
          INSERT INTO "SiteNewsArticle" ("id", "alias", "title", "sefLink", "categoryId", "category", "tags", "featured", "hits",
                                         "introText", "fullText", "publishedAt", "modifiedAt", "imagePath", "imageAlt", "pulledAt")
          VALUES ${values}
    ...
    const [agg] = await prisma.$queryRaw<{ n: bigint; topick: bigint }[]>`
      SELECT count(*)::bigint AS n, count(*) FILTER (WHERE "imagePath" IS NOT NULL AND "imageKey" IS NULL)::bigint AS topick FROM "SiteNewsArticle"`
    const held = Number(agg?.n ?? 0), toUpload = Number(agg?.topick ?? 0)
    ...
    return NextResponse.json({
      ...
      message: written === 0
        ...
        : `Loaded ${written.toLocaleString()} article${written === 1 ? "" : "s"}${skipped ? ` (${skipped} unreadable rows skipped)` : ""}; the Hub now holds ${held.toLocaleString()}. ${toUpload ? `${toUpload.toLocaleString()} cover picture${toUpload === 1 ? "" : "s"} still to upload — choose the collector's pictures folder below.` : "Every cover picture is already in the Hub."}`,
    })
  } catch (e: any) {
    console.error("databases/news/collect error:", e)
    const msg = String(e?.message ?? "")
    return NextResponse.json({ error: /does not exist|relation/i.test(msg) ? "The news table isn't there yet — press Run Migrations on this environment first." : (msg || "Could not read the file") }, { status: 500 })
  }
}
