...
- **Never call it a CRM.** It is the **Vectis Hub** ("the app"). It *began* as a CRM-only tool and grew into the Hub — a broad internal toolset of which the original contacts/CRM piece is now just one section — but Claude kept reflexively calling the whole thing "the CRM"; **this rule exists to stop that habit.** Never use the word "CRM" in UI copy, logs, comments, or memory. (`/crm-settings` reworded to "Department Settings" 2026-06-29. Still visible: the **"CRM" nav section + hub-card label** — the original CRM/contacts section — pending a chosen rename. Its app KEY stays `"CRM"` to preserve per-user/role permissions; only the visible LABELS change.)
