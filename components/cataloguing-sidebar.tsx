  ...
  return (
    <aside className="w-48 h-full bg-white dark:bg-[#1C1C1E] border-r border-gray-200 dark:border-gray-800 flex flex-col py-4">
      <div className="flex items-center justify-between px-4 mb-3">
        ...
        {onClose && (
          <button onClick={onClose} className="text-gray-500 hover:text-gray-900 dark:hover:text-white transition-colors text-lg leading-none">
            ✕
          </button>
        )}
      </div>
          ...
          return (
            <Link key={href} href={href} onClick={onClose}
              className={`flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                active ? "bg-[#2AB4A6] text-gray-900 dark:text-white" : "text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-white"
              }`}>
