...
export const MIGRATIONS = [
  ...
  // JORDAN.SYS meal planner (personal, /jordan) — profiles (BMR numbers + targets) and their plans.
  `CREATE TABLE IF NOT EXISTS "JordanMealProfile" (
    "id"           TEXT NOT NULL PRIMARY KEY,
    "name"         TEXT NOT NULL,
  ...
  `CREATE TABLE IF NOT EXISTS "JordanMealPlan" (
    "id"        TEXT NOT NULL PRIMARY KEY,
    "profileId" TEXT NOT NULL,
    ...
    "createdAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "updatedAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT "JordanMealPlan_profileId_fkey" FOREIGN KEY ("profileId")
      REFERENCES "JordanMealProfile"("id") ON DELETE CASCADE ON UPDATE CASCADE
  )`,
 ...
 ON CONFLICT ("categoryId","name") DO NOTHING`,
  ...
  // 🖼 Databases → Sales (2026-09-22): each sale's BC code and cover picture — the website's
  // "hero" — its address on the site's S3 and our own copy in R2 once the heroes job has run.
