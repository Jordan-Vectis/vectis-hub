...
UNPACKED ITEMS: boxed model railway items (and other boxed items) are photographed with the model and its inner packaging TAKEN OUT of the box and laid beside it. A locomotive or wagon, its plastic cradle or inner tray, and the outer box together are ONE boxed item — never two or three. An empty outer box, a loose inner tray and a model of the thing named on that box belong together; count them once. The picture or label on a box may show a different livery or variant from the model that came out of it — the model itself (and the key points) is what is being sold; never count the box artwork as another item and never describe it as the model.
