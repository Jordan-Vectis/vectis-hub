  ...
  io.on('connection', (socket) => {
    ...
    socket.on('trainer:host', (payload, ack) => {
      ...
      const room = {
        ...
        bidders: new Map(),
        // Paddle numbers start from a random base so they read like real bidder
        // IDs rather than 1, 2, 3.
        paddleSeq: 1000 + Math.floor(Math.random() * 8000),
        touched: Date.now(),
      }
      rooms.set(code, room)
      hostedCode = code
    ...
    socket.on('trainer:join', ({ code, name } = {}, ack) => {
      ...
      const bidder = existing || { paddle: 'B' + room.paddleSeq++, name: cleanName(name) }
    ...
    socket.on('trainer:bid', ({ code, amount } = {}) => {
      ...
      const now = Date.now()
      if (now - lastBidAt < MIN_BID_GAP_MS) return
      lastBidAt = now
    ...
    socket.on('disconnect', () => {
      ...
      if (joinedCode) {
        ...
        if (room && room.bidders.delete(socket.id)) pushRoster(io, room)
      }
    })
  })
}
