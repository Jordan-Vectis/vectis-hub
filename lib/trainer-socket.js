  ...
  io.on('connection', (socket) => {
    ...
    socket.on('trainer:host', (payload, ack) => {
      ...
      const room = {
        ...
        // Which platform skin is hosting — shown on phones and in the lists.
        platform: (payload && payload.platform) === 'vectis' ? 'vectis' : 'saleroom',
        bidders: new Map(),
    ...
    socket.on('trainer:join', ({ code, name } = {}, ack) => {
      ...
      reply({ ok: true, code: room.code, paddle: bidder.paddle, name: bidder.name, platform: room.platform || 'saleroom', state: room.state })
    })
